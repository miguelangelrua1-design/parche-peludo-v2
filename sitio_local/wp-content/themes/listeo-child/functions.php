...
/* ==========================================================================
   PPV2 — CHAT DE CREACIÓN DE LISTADOS (tipo "directorio")
   Shortcode [ppv2_chat_listado]: asistente conversacional (sin IA) que
   recolecta los datos básicos del negocio y crea el listado en estado
   "preview" (el mismo borrador que usa el flujo nativo de Listeo). Al
   terminar envía al dueño a /agregar-listado/?step=submit&listing_id=X,
   donde el formulario oficial aparece PRELLENADO para completar fotos y
   horarios y enviarlo a revisión (los listados nuevos requieren aprobación).
   No se toca listeo-core: se reutiliza su mecanismo de reanudación por
   cookie (listeo-submitting-listing-id/-key + meta _submitting_key).
   ========================================================================== */

/**
 * Árbol de términos (2 niveles) para los botones del chat.
 */
function ppv2_chat_listado_term_tree( $taxonomy ) {
	$tree    = array();
...
/**
 * Shortcode [ppv2_chat_listado] — contenedor + assets del chat.
 */
function ppv2_chat_listado_shortcode() {
	$handle  = 'ppv2-chat-listado';
	$js_path = get_stylesheet_directory() . '/js/ppv2-chat-listado.js';
	if ( file_exists( $js_path ) ) {
		wp_enqueue_script(
			$handle,
			get_stylesheet_directory_uri() . '/js/ppv2-chat-listado.js',
			array( 'jquery' ),
			filemtime( $js_path ),
			true
		);
		wp_localize_script( $handle, 'ppv2ChatListado', array(
			'ajaxUrl'    => admin_url( 'admin-ajax.php' ),
			'nonce'      => wp_create_nonce( 'ppv2_chat_listado' ),
			'loggedIn'   => is_user_logged_in(),
			'loginUrl'   => wp_login_url( get_permalink() ),
			'categories' => ppv2_chat_listado_term_tree( 'listing_category' ),
			'regions'    => ppv2_chat_listado_term_tree( 'region' ),
		) );
	}
	return '<div id="ppv2-chat-listado" class="ppv2-chat" aria-live="polite"></div>';
}
add_shortcode( 'ppv2_chat_listado', 'ppv2_chat_listado_shortcode' );

/**
 * AJAX: crea el listado borrador con los datos del chat.
 */
function ppv2_chat_listado_create() {
	...
	if ( get_transient( 'ppv2_chat_listado_lock_' . $user_id ) ) {
		wp_send_json_error( array(
			'code'    => 'rate',
			'message' => __( 'Acabas de crear un listado. Espera un minuto e inténtalo de nuevo.', 'listeo' ),
		) );
	}
	...
	if ( '' === $title || mb_strlen( $description ) < 40 ) {
		wp_send_json_error( array(
			'code'    => 'invalid',
			'message' => __( 'Falta el nombre del negocio o la descripción es muy corta.', 'listeo' ),
		) );
	}
...
add_action( 'wp_ajax_ppv2_chat_listado_create', 'ppv2_chat_listado_create' );
add_action( 'wp_ajax_nopriv_ppv2_chat_listado_create', 'ppv2_chat_listado_create' );
